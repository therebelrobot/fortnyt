...
import type { Account, Item, Person, Reserve, Txn } from '../src/shared/types';
...
function data(items: Item[], txns: Txn[], today: string, accounts: Account[] = [], reserves: Reserve[] = []): EngineData {
  return {
    pay,
    today,
    items,
...
describe('due lines and attribution', () => {
  ...
  const rent = item({ name: 'Rent', kind: 'expense', amountCents: 140000, cadence: 'monthly', dayOfMonth: 1 });
  ...
  it('computes the leftover from counted income, commitments, and unplanned spending', () => {
    const phone = item({ name: 'Phone', kind: 'expense', amountCents: 5500, cadence: 'monthly', dayOfMonth: 10 });
    const txns = [
...
describe('reserve (sinking fund) lines', () => {
  ...
  it('pools several lines into one named reserve, with contributions and direct withdrawals', () => {
    ...
    const topUp = item({ name: 'Extra to car fund', kind: 'expense', amountCents: 5000, cadence: 'monthly', dayOfMonth: 10, reserveId: 1 });
    const txns = [
...
describe('people', () => {
  ...
  const rent = item({ name: 'Rent', kind: 'expense', amountCents: 100000, cadence: 'monthly', dayOfMonth: 10 });
  const txns = [
